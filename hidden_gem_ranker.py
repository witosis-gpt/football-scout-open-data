...
def main():
    ...
    pool=df[df['main_position'].isin(['Attack','Midfield'])].copy() if 'main_position' in df.columns else df.copy()
    ...
    pool['production_pct']=pool.groupby(pos_key)['ga_per90'].rank(pct=True,method='average').fillna(0.5) if pos_key else pct(pool['ga_per90'],ascending=True)
    ...
    pool['hidden_gem_score']=(0.38*pool['spike_score']+0.22*pool['production_pct']+0.16*pool['age_upside']+0.14*pool['affordability']+0.10*pool['minutes_confidence'])*100
    pool['value_band']=pd.cut(pool['market_value_eur'],bins=[-1,500000,2000000,5000000,10000000,float('inf')],labels=['<=0.5m','0.5-2m','2-5m','5-10m','10m+'])

    cols=[c for c in [
        'player_id','player_name','age','citizenship','foot','position','main_position','current_club_name',
        'team_name','competition_name','season','minutes_played_num','goals_per90','assists_per90','ga_per90',
        'market_value_eur','value_band','spike_score','production_pct','age_upside','affordability',
        'minutes_confidence','hidden_gem_score'
    ...
    cheap=pool[(pool['market_value_eur'].isna()) | (pool['market_value_eur']<=5_000_000)].copy().sort_values('hidden_gem_score',ascending=False)[cols]
    ...
    pd.DataFrame([{'input_rows':len(df),'senior_attack_midfield_900min_pool':len(pool),'under_5m_or_unknown_pool':len(cheap),'status':'ok','note':'Senior competitions only; heuristic based on production, age, minutes and market value; no dribble/pass/carry event data.'}]).to_csv(OUT/'hidden_gem_status.csv',index=False)
